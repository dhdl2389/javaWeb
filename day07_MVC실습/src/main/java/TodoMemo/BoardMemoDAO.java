...
public class BoardMemoDAO {
	...
	public BoardMemo updateMemo(String m_code, String m_contents) {
		Connection con = dbcon();
		String sql = "update memo_tbl set m_contents = ? where m_code=?";
		PreparedStatement pst = null;
		BoardMemo updateMemo = null;

		try {
			pst = con.prepareStatement(sql);
			pst.setString(1, m_contents);
			pst.setString(2, m_code);

			int rowsAffected = pst.executeUpdate(); // 업데이트 쿼리 실행 후 영향을 받은 행 수 반환

			if (rowsAffected > 0) {
				// 업데이트가 성공하면 해당 메모를 가져와서 객체를 생성
				updateMemo = new BoardMemo(m_code, m_contents);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			close(con, pst); // ResultSet은 사용하지 않으므로 null 전달
		}
		return updateMemo;
	}
